...
// The tests below require Scratch Arduino Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying Scratch Arduino Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('Scratch Arduino Link'); // Scratch Arduino Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying Scratch Arduino Link is unavailable (BT)', async () => {
        await loadUri(uri);
